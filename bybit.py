...
initial_cash_usd  = 100     # 初期予算（USDT）
...
def fetch_btc_price():
    try:
        price = exchange.fetch_ticker("BTC/USDT").get('last')
...
# === 成行注文 ===
def execute_market_buy(usd_amt: float):
    try:
        if usd_amt < MIN_USD_ORDER:
            raise RuntimeError(f"注文量不足: {usd_amt}")
        bal = get_usd_balance()
        if bal < usd_amt:
            raise RuntimeError(f"残高不足: {bal}")
        price = fetch_btc_price()
        btc_amt = round(usd_amt / price, 8)
        ord = exchange.create_order("BTC/USDT", "market", "buy", btc_amt)
...
def execute_market_sell(btc_amt: float):
    try:
        ...
        exchange.create_order("BTC/USDT", "market", "sell", btc_amt)
...
try:
    ...
    print("価格取得失敗で終了")
    exit(1)
print(f"FGI={fgi}, BTC価格={price}, USD残高={usd_bal}, BTC残高={btc_bal}")
...
if fgi <= fear_threshold and not already_bought_today(trades):
    ...
    alloc = initial_cash_usd / buy_divisions * mult
    ...
    if amt:
        ...
        trades.to_csv(log_file, index=False)
        print(f"買い記録: {amt} BTC @ {buy_pr}")
    else:
        print("[スキップ] 買い記録なし")
else:
    print("[スキップ] 購入条件未達または本日購入済み")
...
for i, row in trades.iterrows():
    ...
    if ratio >= sell_full_target:
        ...
        if order:
            ...
            trades.to_csv(log_file, index=False)
            print(f"全売却 id={row.id} {row.remaining_amount} BTC")
